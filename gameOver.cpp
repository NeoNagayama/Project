...
void EndlessGameOver(float round)
{
    ...
    GameOverText.DrawTextWithSort(0, 1920, "You Survived %.f Round", titleFontHandle, SORT_CENTER, 350, true,
                                  GetColor(255, 255, 0), GetColor(50, 50, 50), round - 1);
    HighScoreText.DrawTextWithSort(0, 1920, "HighScore:%.fRound", fontHandle, SORT_CENTER, 550, true,
                                   GetColor(255, 255, 0), GetColor(50, 50, 50), highScore);
    // スペースキーが押されたら
    if (Input_GetKeyboardDown(KEY_INPUT_SPACE))
    {
        // 場面の遷移を始める
