...
void instruction::inst()
{
    ...
    if (showTimer.MeasureTimer(0.5f))  // 0.5秒たったら
    {
        ...
        // 次に進むための操作を表示
        text.DrawTextWithSort(0, 1920, "PRESS SPACE TO START", BiggerFontHandle, SORT_CENTER, 900, FALSE,
...
void instruction::choiceDifficulty(int scenea)
{
    ...
    if (dif == 0)
    {
        info.DrawTextWithSort(90, 1920, "障害物にぶつかっても即死しません\nハードよりもダメージが少なくなっています",
                              biggerJpFontHandle, SORT_LEFT, 800, true, GetColor(255, 255, 255));
    }
    else
    {
