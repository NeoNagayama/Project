...
void fontLoad()
{
    ...
    CountDownFontHandle =
        CreateFontToHandle("fonts/NotoSansJP-VariableFont_wght.ttf", 150, 5, DX_FONTTYPE_ANTIALIASING);
}

void UIText::DrawTextWithSort(int xleft, int xright, std::string text, int FontHandle, int sort, int y, bool shadow,
                              unsigned int FontColor, unsigned int shadowcolor, float value)
{
    /* @brief テキストの描画幅 */
    int width;
    // テキストのそろえ方別の処理
    ...
    {
        // 描画幅を求める
        width = GetDrawFormatStringWidthToHandle(FontHandle, const_cast<char *>(text.c_str()),
                                                 (int)strlen(const_cast<char *>(text.c_str())), value);
        ...
        if (shadow)
        {
            ...
            DrawFormatStringToHandle(width + 3, y + 2, shadowcolor, FontHandle, const_cast<char *>(text.c_str()),
                                     value);
    ...
    else
    {
        ...
        {
            SetDrawBlendMode(DX_BLENDMODE_ALPHA, 128);
            DrawFormatStringToHandle(xleft + 3, y + 2, shadowcolor, FontHandle, const_cast<char *>(text.c_str()),
                                     value);
            SetDrawBlendMode(DX_BLENDMODE_NOBLEND, 0);
        }
...
bool UIText::fadeInText(int xleft, int xright, std::string text, int FontHandle, int sort, int y, bool shadow,
                        unsigned int FontColor, unsigned int shadowcolor, float value, float time)
{
    int width;
